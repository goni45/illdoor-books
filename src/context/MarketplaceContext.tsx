...
export const MarketplaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshBooks = useCallback(async () => {
    const { data: booksData, error } = await supabase
      .from('books')
      .select('*, profiles(id, full_name, avatar_url, is_verified, is_admin, rating, total_sales, total_purchases, institute, department, semester, created_at), book_images(url)')
    ...
    setBooks(institute ? mapped.filter((book) => book.seller.institute === institute) : mapped);
  ...
  const refreshOrders = useCallback(async () => {
    ...
    let ordersQuery = supabase
      ...
      .select(`
        *,
        books ( *, book_images(url), profiles(id, full_name, avatar_url, is_verified, is_admin, rating, total_sales, total_purchases, institute, department, semester, created_at) ),
        buyer:profiles!buyer_id(id, full_name, avatar_url, is_verified, is_admin, rating, total_sales, total_purchases, institute, department, semester, created_at),
        seller:profiles!seller_id(id, full_name, avatar_url, is_verified, is_admin, rating, total_sales, total_purchases, institute, department, semester, created_at),
        pickup_points(*)
      `);
    if (!isAdmin) {
      ordersQuery = ordersQuery.or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`);
    }
    ...
    if (error) {
      ...
      if (!isAdmin) {
        fallbackQuery = fallbackQuery.or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`);
      }
    ...
    setOrders(ordersWithDetails);
  }, [user, books, pickupPoints, currentUser, isAdmin]);
  ...
  const addBookListing = useCallback(async (
    ...
    if (error || !inserted) {
      ...
      throw new Error(error?.message || 'Could not publish listing. Please try again.');
    }
  ...
  const updateBookStatus = useCallback(async (bookId: string, availability: BookListing['availability']) => {
    ...
    if (current.availability === 'Sold' || current.availability === 'Reserved') {
      throw new Error('Sold or reserved listings cannot be manually reactivated.');
    }
    if (!['Available', 'Inactive'].includes(availability)) {
      throw new Error('Only Available and Inactive can be selected manually.');
    }

    const { error } = await supabase.from('books').update({ availability }).eq('id', bookId);
  ...
  const createOrder = useCallback(async (bookId: string, pickupPointId: string): Promise<{ orderId: string; error: string | null }> => {
    ...
    if (targetBook.seller.id === user.id) {
