...
export const SellBookPage: React.FC = () => {
  ...
  const handleSubmitListing = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!newId) {
        setSubmitError('Listing could not be published. Please try again.');
        return;
      }
      setSubmittedId(newId);
    } catch (err) {
      setSubmitError(err instanceof Error ? err.message : 'Listing could not be published.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <form onSubmit={handleSubmitListing} className="space-y-5 animate-in fade-in duration-200">
            ...
            <div className="pt-4 flex items-center justify-between">
              ...
              <Button
                ...
                disabled={submitting}
              >
                <span>{submitting ? 'Publishing…' : `Publish Listing (৳${sellingPrice})`}</span>
