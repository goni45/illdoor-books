...
export const HomePage: React.FC = () => {
  ...
  const [mobileTab, setMobileTab] = useState<'savings' | 'matcher' | 'safety'>('savings');

  // Card 2 interactive form state: Syllabus book matcher
  const [filterDept, setFilterDept] = useState('Electronics Technology');
  const [filterSemester, setFilterSemester] = useState('All Semesters');
  const [filterCode, setFilterCode] = useState('26641');
  const [targetBudget, setTargetBudget] = useState('180');
  const [savedBanner, setSavedBanner] = useState(false);

  // Card 3 interactive state: Protected seller contact
  ...
  const featuredBooks = React.useMemo(() => {
    return books.filter((book)=>(book.sellerCount??0)>0).sort((a,b)=>getBookListingRank(a)-getBookListingRank(b)).slice(0,4);
  }, [books]);
  const highSavingsBooks = books.filter((book)=>(book.availableStock??0)>0)
    .sort((a, b) => b.savings - a.savings)
    .slice(0, 4);

  const departments = React.useMemo(() => {
    const catalog = new Map<string, { name: string; ids: Set<string>; code: string }>();
    for (const book of books) {
      for (const entry of book.curriculumEntries ?? []) {
        const current = catalog.get(entry.department) ?? { name: entry.department, ids: new Set<string>(), code: entry.technologyCode };
        current.ids.add(book.id);
        catalog.set(entry.department, current);
      }
    }
    return Array.from(catalog.values()).map((entry) => ({
      name: entry.name,
      count: `${entry.ids.size} Books`,
      code: `Technology ${entry.code}`,
    }));
  }, [books]);
  ...
  const handleResetForm = () => {
    setFilterDept('Computer Science & Technology');
    ...
    <div className="w-full">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-between">
          ...
          <div className="flex flex-col items-center px-3 sm:px-4 pt-3 sm:pt-10 md:pt-12 pb-2 sm:pb-6 text-center shrink-0 my-auto">
            ...
            >
              ...
              students. Verified same-institute sellers, direct contact, and inspect-before-payment safety.
            </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between text-[13px] font-semibold">
                      <span className="text-[#ef4d23]">
                        {savingsToggle === 'savings' ? 'শিক্ষার্থীদের সাশ্রয়' : 'বই হস্তান্তর'}
                      </span>
                      <span className="text-neutral-500 font-medium">চলতি সেমিস্টার</span>
                    </div>

                    <div className="flex items-baseline gap-2 mt-2">
                      <span className="text-[26px] sm:text-[28px] font-bold text-neutral-900 tracking-tight">
                        {savingsToggle === 'savings' ? '৳১৪,৮৫০' : '৪৩৬টি বই'}
                      </span>
                      <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200/80 rounded-full px-2 py-0.5 text-[11px] font-semibold">
                        ...
                        <span>{savingsToggle === 'savings' ? 'নিলক্ষেতের চেয়ে -৬৫%' : 'এই সপ্তাহে +১২৮'}</span>
                      </span>
                    </div>

                    <p className="text-[11px] text-neutral-500 mt-0.5 font-medium">
                      {savingsToggle === 'savings'
                        ? 'পলিটেকনিক ডিপ্লোমা শিক্ষার্থীদের মোট সাশ্রয়'
                        : 'ক্যাম্পাসে সক্রিয় ব্যবহৃত বই বিনিময়'}
                    </p>

                    <div className="text-center text-xs font-semibold text-neutral-600 mt-3 sm:mt-4 mb-1">
                      সেমিস্টার লক্ষ্য অর্জিত
                    </div>
                    ...
                    <div className="w-full max-w-[220px] sm:max-w-[260px] mx-auto flex flex-col">
                      ...
                      <div className="flex justify-between items-center text-[11px] text-neutral-500 px-3 -mt-1 font-semibold">
                        <span>{savingsToggle === 'savings' ? '৳৮.৫হাজার' : '২০০'}</span>
                        <span>{savingsToggle === 'savings' ? '৳১৫হাজার' : '৫০০'}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div className={`glass-card-white-inner rounded-xl sm:rounded-2xl p-3.5 sm:p-5 flex-col gap-2 sm:gap-3 shadow-xs relative ${mobileTab === 'matcher' ? 'flex' : 'hidden sm:flex'}`}>
                  ...
                  <div className="flex items-center justify-between text-[13px] font-semibold mb-0.5">
                    <span className="text-[#ef4d23]">বই খোঁজক (ম্যাচার)</span>
                    <span className="text-neutral-500 font-medium">বিটিইবি সিলেবাস</span>
                  </div>

                  <div className="flex flex-col gap-1">
                    <label className="text-[12px] text-neutral-700 font-semibold">টেকনোলজি / বিভাগ</label>
                    <div className="relative">
                      ...
                      >
                        <option value="Electronics Technology">ইলেকট্রনিক্স টেকনোলজি (৬৮)</option>
                        <option value="Computer Science & Technology">কম্পিউটার সায়েন্স অ্যান্ড টেকনোলজি (৮৫)</option>
                        <option value="Civil Technology">সিভিল টেকনোলজি (৬৪)</option>
                        <option value="Electrical Technology">ইলেকট্রিক্যাল টেকনোলজি (৬৭)</option>
                        <option value="Mechanical Technology">মেকানিক্যাল টেকনোলজি (৭০)</option>
                      </select>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[12px] text-neutral-700 font-semibold">উদ্দিষ্ট সেমিস্টার</label>
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none border border-neutral-200/90 bg-white/95 rounded-lg px-3 py-1.5 text-xs sm:text-sm text-neutral-800 hover:border-neutral-400 transition-colors cursor-pointer outline-none pr-8 font-medium shadow-xs"
                      >
                        <option value="All Semesters">সব সেমিস্টার</option>
                        <option value="1st - 2nd Semester">১ম - ২য় সেমিস্টার</option>
                        <option value="3rd - 4th Semester">৩য় - ৪র্থ সেমিস্টার</option>
                        <option value="5th - 6th Semester">৫ম - ৬ষ্ঠ সেমিস্টার</option>
                        <option value="7th - 8th Semester">৭ম - ৮ম সেমিস্টার</option>
                      </select>
                      <ChevronDown className="w-3.5 h-3.5 text-neutral-500 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[12px] text-neutral-700 font-semibold">বিষয় কোড বা বইয়ের নাম</label>
                    <div className="relative flex items-center border border-neutral-200/90 bg-white/95 rounded-lg px-3 py-1 focus-within:border-neutral-400 transition-colors shadow-xs">
                      ...
                      <input
                        ...
                        placeholder="যেমন: 26642 বা ডেটা স্ট্রাকচার"
                  ...
                  <div className="flex flex-col gap-1">
                    <label className="text-[12px] text-neutral-700 font-semibold">সর্বোচ্চ বাজেট (৳)</label>
                    <div className="relative flex items-center border border-neutral-200/90 bg-white/95 rounded-lg px-3 py-1 focus-within:border-neutral-400 transition-colors shadow-xs">
                      <span className="text-neutral-400 text-xs mr-1.5 font-mono select-none">৳</span>
                      <input
                        type="number"
                ...
                {/* Card 3 — Protected Seller Contact */}
                <div className={`glass-card-white-inner rounded-xl sm:rounded-2xl p-3.5 sm:p-5 shadow-xs flex-col justify-between ${mobileTab === 'safety' ? 'flex' : 'hidden sm:flex'}`}>
                  <div>
                    <div className="flex items-center justify-between text-[13px] font-semibold">
                      <span className="text-[#ef4d23]">যাচাইকৃত যোগাযোগ</span>
                      <span className="text-neutral-500 font-medium">সরাসরি লেনদেন</span>
                    </div>

                    <div className="flex items-baseline gap-2 mt-2">
                      <span className="text-[26px] sm:text-[28px] font-bold text-neutral-900 tracking-tight">
                        {handoverToggle === 'booths' ? 'একই ক্যাম্পাস' : 'নিজেদের সিদ্ধান্ত'}
                      </span>
                      <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200/80 rounded-full px-2 py-0.5 text-[11px] font-semibold">
                        <TrendingUp className="w-3 h-3 shrink-0" />
                        <span>ইনস্টিটিউট মিলেছে</span>
                      </span>
                    </div>

                    <p className="text-[11px] text-neutral-500 mt-0.5 font-medium">
                      {handoverToggle === 'booths'
                        ? 'ক্যাম্পাসের জনসমাগমপূর্ণ স্থানে দেখা করুন'
                        : 'বই দেখে তারপর সরাসরি মূল্য পরিশোধ করুন'}
                    </p>

                    <div className="text-center text-xs font-semibold text-neutral-600 mt-3 sm:mt-4 mb-1">
                      যোগাযোগ ও সংগ্রহের সুরক্ষা
                    </div>
                    ...
                    <div className="w-full max-w-[220px] sm:max-w-[260px] mx-auto flex flex-col">
                      ...
                      />
                      <div className="flex justify-between items-center text-[11px] text-neutral-500 px-3 -mt-1 font-semibold">
                        <span>প্রোফাইল যাচাই</span>
                        <span>আগাম টাকা নয়</span>
                      </div>
                    </div>
                  </div>
      ...
      <div className="mt-8 sm:mt-12 md:mt-14 space-y-8 sm:space-y-14 pb-12 px-2 sm:px-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg sm:text-xl font-bold text-[#0b0f1a] tracking-tight">
                টেকনোলজি / বিভাগ অনুযায়ী খুঁজুন
              </h2>
              <p className="text-xs sm:text-sm text-neutral-500">
                পলিটেকনিক সিলেবাস অনুসারে সাজানো পাঠ্যবই ব্রাউজ করুন
              </p>
            </div>
            <button
              onClick={() => {
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-[#0b0f1a] tracking-tight">
                  সম্প্রতি যুক্ত বইসমূহ
                </h2>
                <p className="text-xs sm:text-sm text-neutral-500">
                  ক্যাম্পাস শিক্ষার্থীদের নতুন লিস্টিং ও ফ্রেশ কালেকশন
                </p>
              </div>
            </div>
            <button
        ...
        {featuredBundles.length > 0 && <section className="space-y-4">
          <div className="flex items-center justify-between"><div className="flex gap-2 items-center"><Layers3 className="w-5 h-5 text-[#ef4d23]"/><div><h2 className="text-lg sm:text-xl font-bold">সম্পূর্ণ সেমিস্টার বইয়ের সেট</h2><p className="text-xs sm:text-sm text-neutral-500">আপনার ডিপার্টমেন্ট ও ইনস্টিটিউটের সম্পূর্ণ বইয়ের বান্ডেল</p></div></div><button onClick={() => setActiveView('semester-bundles')} className="text-xs sm:text-sm font-semibold hover:text-[#ef4d23]">সব দেখুন →</button></div>
          <div className="grid md:grid-cols-3 gap-4">{featuredBundles.map((bundle) => <button key={bundle.id} onClick={() => setActiveView('semester-bundles')} className="text-left bg-white rounded-3xl border p-5 hover:border-[#ef4d23] transition-colors"><span className="inline-flex px-3 py-1 rounded-full bg-[#0b0f1a] text-white text-[11px] font-bold">ফুল সেট মাত্র</span><h3 className="text-xl font-bold mt-3">{bundle.semester}</h3><p className="text-xs text-neutral-500">{bundle.items.length}টি বই • {bundle.seller.name}</p><div className="flex justify-between items-end mt-5"><span className="text-xs text-neutral-500">{bundle.pickupPointName}</span><b className="text-xl">৳{bundle.sellingPrice}</b></div></button>)}</div>
        </section>}
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-[#ef4d23] text-xs font-semibold uppercase tracking-wider mb-2">
                ...
                <span>শিক্ষার্থীদের বইয়ের চাহিদাবোর্ড</span>
              </div>
              <h2 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">
                শিক্ষার্থীরা এই বইগুলো খুঁজছেন
              </h2>
              <p className="text-xs sm:text-sm text-neutral-400 mt-1 max-w-xl">
                আপনার কাছে কি এই পাঠ্যবইগুলো আছে? লিস্টিং করে সহজে বিক্রি করুন, অথবা আপনার প্রয়োজনীয় বইয়ের জন্য রিকোয়েস্ট পোস্ট করুন।
              </p>
            </div>
          ...
          {bookRequests.filter((r) => r.status === 'open').length > 0 ? (
            ...
            <div className="bg-neutral-800/60 rounded-2xl border border-neutral-700/60 p-6 text-center relative z-10">
              <p className="text-xs sm:text-sm text-neutral-300 mb-3">
                বর্তমানে কোনো অপূর্ণ অনুরোধ নেই। কোনো সেমিস্টারের বই প্রয়োজন?
              </p>
              <button
                onClick={() => {
                  setActiveView('requests');
              ...
              >
                ...
                <span>বইয়ের জন্য অনুরোধ পোস্ট করুন</span>
              </button>
            </div>
          )}
        </section>
        ...
        <section className="bg-white rounded-3xl border border-[#e5e5e5] p-6 sm:p-8 shadow-xs">
          <div className="max-w-2xl mb-6">
            <span className="text-xs font-bold uppercase tracking-wider text-[#ef4d23]">
              সরাসরি যোগাযোগের নিরাপত্তা
            </span>
            <h2 className="text-lg sm:text-xl font-bold text-[#0b0f1a] tracking-tight mt-0.5">
              নিরাপদ দেখা করার প্রস্তাবিত স্থানসমূহ
            </h2>
            <p className="text-xs sm:text-sm text-neutral-500 mt-1">
              বিক্রেতার সাথে সময় ও স্থান ঠিক করুন, ক্যাম্পাসের জনসমাগমপূর্ণ স্থানে বই যাচাই করুন এবং বই হাতে পাওয়ার পর মূল্য পরিশোধ করুন। Illdoor পেমেন্টের মধ্যস্থতা করে না।
            </p>
          </div>
