...
## Contact-first addendum (supersedes checkout for new activity)

For newly initiated activity, Illdoor is a discovery and protected-contact product—not a payment, escrow, order, refund, delivery, or pickup intermediary. Previous order sections apply only to historical records.

A signed-in buyer browses their department catalog, selects a same-institute seller offer/bundle, reviews safety guidance, and reveals only the seller-approved contact method through a secured RPC. Raw contact fields never appear in anonymous/public profile reads, catalog joins, or rendered markup before authorization. Reveal validates target availability and bundle membership, blocks self-contact, enforces seller consent, serializes concurrent attempts, rate-limits and audits access, and notifies the seller.

Sellers control contact preferences and manually manage listing/bundle lifecycle. Active historical orders prevent non-admin status overrides. New UI never invokes single-book checkout, semester-bundle checkout, or bundle-request conversion. Existing cancellation, completion, dispute, and completed-order review permissions remain unchanged for legacy records.

---

## 27. Browser URL Routing & Navigation Architecture (Implemented)

The marketplace uses `react-router-dom` (v7+) with HTML5 browser history (`BrowserRouter`), replacing in-memory `activeView` state as the source of truth while maintaining backward-compatible context bindings.

### 27.1 Route Map

| Path | Component / View | Access Level | Description |
| --- | --- | --- | --- |
| `/` | `HomePage` | Public | Landing page, hero, quick category & department links, recent books |
| `/books` | `BrowsePage` | Public (Personalized for logged-in students) | Searchable catalog synchronized with query parameters (`search`, `semester`, `subject`, `sort`) |
| `/books/:bookId` | `BookDetailsPage` | Public | Book model details, publication editions, and same-institute seller offers |
| `/bundles` | `SemesterBundlesPage` | Public (Personalized for logged-in students) | Complete-semester bundle offerings |
| `/sell` | `SellBookPage` | Protected (Authenticated) | Individual book and semester bundle listing creation flow |
| `/wishlist` | `WishlistPage` | Public / Context-aware | Saved book models |
| `/requests` | `RequestsPage` | Public | Book request board |
| `/notifications` | `NotificationsPage` | Protected (Authenticated) | Order, contact interest, and system notification center |
| `/profile` | `ProfilePage` | Protected (Authenticated) | User profile, contact preferences, and student ID verification |
| `/orders` | `OrdersPage` | Public / Context-aware | Clearly labelled legacy order history overview |
| `/orders/:orderId` | `OrdersPage` | Public / Context-aware | Detail view of a specific historical order |
| `/admin` | `AdminDashboard` | Protected (Admin only) | Operations console gated by `profiles.is_admin` |
| `*` | `NotFoundPage` | Public | Friendly 404 page with navigation to Home and Browse |

### 27.2 Browser History & Scroll Restoration Behavior
- **Forward & Normal Navigation (`PUSH` / `REPLACE`):** Automatically scrolls window to top (`(0, 0)`) via `ScrollManager`.
- **Browser Back & Forward (`POP`):** Preserves native browser scroll position.
- **Search & Filter Parameter Sync:** Catalog query params (`/books?search=...&semester=...&subject=...&sort=...`) synchronize bidirectionally with browse state without reloading or resetting history unexpectedly.
- **Backward-Compatible Redirects:** Legacy query URLs (`/?book=:id`, `/?order=:id`, and `/?view=:view`) are automatically redirected to standard paths (`/books/:id`, `/orders/:id`, `/view`) using HTTP/Router `replace: true`.

### 27.3 Protected Route Rules
- **Authenticated Routes (`/sell`, `/notifications`, `/profile`):** Wrapped by `<ProtectedRoute>`. Unauthenticated visitors see the inline authentication prompt with option to sign in, rather than broken state or blank screen.
- **Admin Route (`/admin`):** Gated by `profiles.is_admin`. Non-admin accounts and unauthenticated users receive an access-restricted notice.
- **Server-Side Authorization Invariance:** Client-side routing guards are purely navigational UX; Supabase Row Level Security (RLS) policies and RPC `SECURITY DEFINER` permission checks remain the non-negotiable security boundary.

### 27.4 Deep-Link & Sharing Specifications
- Book detail share action generates canonical URL: `${origin}/books/${book.id}`.
- Shared URLs open directly on `BookDetailsPage` by reading `useParams<{ bookId: string }>()`.
- Legacy `/?book=:id` links cleanly transition to `/books/:id`.

### 27.5 Hosting Rewrite Requirements
- **Vercel (`vercel.json`):** Configured with single-page application rewrite:
  ```json
  {
    "rewrites": [{ "source": "/(.*)", "destination": "/index.html" }]
  }
  ```
- **Netlify / Static Hosts (`public/_redirects`):** Configured with SPA rewrite rule:
  ```text
  /* /index.html 200
  ```
- This ensures direct visits, browser refreshes, and deep links on nested paths (e.g., `/books/:id`, `/orders/:id`) correctly return `index.html`.

### 27.6 Security & Contact-First Architecture Confirmation
- Sensitive state—including phone numbers, WhatsApp numbers, passwords, auth tokens, and session secrets—is NEVER serialized into URLs, query parameters, or browser history.
- The `ContactSellerModal` remains client-side modal state and is never exposed as a public route.
