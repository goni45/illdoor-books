...
CREATE TABLE IF NOT EXISTS public.book_requests (
  ...
  fulfilled_by_book_id UUID REFERENCES public.books(id) ON DELETE SET NULL,
  created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
  ...
  IF clean_code IS NOT NULL AND clean_code <> '' THEN
    ...
    LOOP
      INSERT INTO public.notifications (
        ...
        'A book matching your request for ' || NEW.subject_code || ' (' || NEW.title || ') was just listed for ৳' || NEW.price || '.',
        'system',
        '/book/' || NEW.id,
        false,
        NOW()
