...
function mapDbBookToListing(row: Record<string, unknown>): BookListing {
  ...
  const offers: SellerListing[] = rawOffers.map((offer) => {
    ...
    return {
      ...
      conditionDetails: (offer.condition_details as string) || '',
      ...
      pickupPointId: offer.pickup_point_id as string, pickupPointName: offer.pickup_point_name as string,
  ...
  return {
    ...
    seller: primary?.seller ?? emptySeller, pickupPointId: primary?.pickupPointId ?? '', pickupPointName: primary?.pickupPointName ?? '',
...
function applyVisibleOffers(book:BookListing,offers:SellerListing[]):BookListing{const sorted=[...offers].sort((a,b)=>a.sellingPrice-b.sellingPrice);const active=sorted.filter(o=>o.availability==='Available');const primary=active[0]??sorted[0];const publication=primary?.publication??book.publication;return{...book,offers:sorted,availableStock:active.length,sellerCount:new Set(active.map(o=>o.seller.id)).size,lowestPrice:active[0]?.sellingPrice,seller:primary?.seller??book.seller,condition:primary?.condition??'Used',conditionDetails:primary?.conditionDetails??'No active seller offers.',originalPrice:primary?.originalPrice??0,sellingPrice:primary?.sellingPrice??0,savings:primary?.savings??0,availability:active.length?'Available':'Inactive',publication,images:[getPublicationCover(book,publication)],pickupPointId:primary?.pickupPointId??'',pickupPointName:primary?.pickupPointName??''}}
...
function mapDbOrderToOrder(row: Record<string, unknown>, book: BookListing, buyer: StudentUser, seller: StudentUser, pickup: PickupPoint): Order {
  ...
  const statusLabels: Partial<Record<OrderStatus, { label: string; note: string }>> = {
    placed: { label: 'Order Placed', note: `Pickup: ${pickup.name}` },
...
export const MarketplaceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateBookStatus=useCallback(async(listingId:string,availability:BookListing['availability'])=>{
    if(availability!=='Available'&&availability!=='Inactive')throw new Error('Only Available or Inactive is allowed.'); const {error}=await supabase.rpc('set_my_listing_availability',{p_listing_id:listingId,p_availability:availability}); if(error) throw new Error(error.message); await refreshBooks();
  },[refreshBooks]);
